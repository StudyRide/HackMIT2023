...
router.post("/findRides", (req: Request, res: Response) => {
  ...
  Ride.find({})
    .then((rides) => {
      ...
      foundRides.forEach((ride: any) => {
        ...
        // toReturn.push(ride);
        if (
          searchRegex.test(ride.destination) ||
          searchRegex.test(ride.origin) ||
          searchRegex.test(ride.driver)
        ) {
  ...
  Ride.find({})
    ...
    .catch((err) => {
      ...
      res
        .status(500)
        .send(
          "An error occurred while trying to get rides specific to the user"
        );
    });
});
  ...
  const toAddRides = [
    {
      ...
      origin: "new york",
      destination: "boston",
      timeOfCreation: "2023-09-17T03:34:24.826Z",
      rideDepartureTime:
    ...
    },
    {
      ...
      timeOfCreation: "2023-09-17T12:51:29.278Z",
      rideDepartureTime: "10:51",
      rideReturnTime: "00:51",
      cost: 25,
      requests: [],
      __v: 0,
    },
